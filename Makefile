...
BUILD_OUT_DIR := $(strip build)

all:
	mkdir -p $(BUILD_OUT_DIR)/include/uWS
	$(CXX) $(CPPFLAGS) $(CFLAGS) $(CPP_SHARED) -s -o $(BUILD_OUT_DIR)/libuWS.so
	cp src/*.h $(BUILD_OUT_DIR)/include/uWS
